...
public class IdWorker {
    ...
    private long workerId;    //工作id，下面两个每个5位，加起来就是10位的工作机器id
    ...
    private long workerIdBits = 5L;     //长度为5位

    private long datacenterIdBits = 5L;

    private long maxWorkerId = -1L ^ (-1L << workerIdBits);     //最大值
    ...
    public static IdWorker getSingleton(long workerId, long datacenterId, long sequence) {
        if (singleton == null) {
            synchronized (IdWorker.class) {
                if (singleton == null) {
                    singleton = new IdWorker(workerId, datacenterId, sequence);
                }
            }
        }
        return singleton;
